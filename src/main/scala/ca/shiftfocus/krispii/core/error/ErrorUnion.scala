...
sealed trait PaymentRequiredT extends ErrorUnion {
  case class PaymentRequired(message: String) extends Fail
}

sealed trait ExpirationT extends ErrorUnion {
  case class Expiration(message: String) extends Fail
}
