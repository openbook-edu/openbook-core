...
class GfileRepositoryPostgres extends GfileRepository with PostgresRepository[Gfile] {
  ...
  override def constructor(row: RowData): Gfile = {
    Gfile(
      ...
      permissionId = Option(row("permission_id").asInstanceOf[String]),
  ...
  val Table = "gfiles"
  val Fields = "id, work_id, file_id, mime_type, file_type, file_name, embed_url, url, shared_email, permission_id, revision_id, created_at"
  ...
  val Update =
    s"""
       |UPDATE $Table
       |SET shared_email = ?, permission_id = ?, revision_id = ?
       |WHERE id = ?
       |RETURNING $Fields
    """.stripMargin
  ...
  def insert(gfile: Gfile)(implicit conn: Connection): Future[\/[RepositoryError.Fail, Gfile]] = {
    val params = Seq[Any](
      ...
      gfile.fileName, gfile.embedUrl, gfile.url, gfile.sharedEmail, gfile.permissionId, gfile.revisionId, gfile.createdAt
    )
  ...
  def update(gfile: Gfile)(implicit conn: Connection): Future[\/[RepositoryError.Fail, Gfile]] = {
    val params = Seq[Any](gfile.sharedEmail, gfile.permissionId, gfile.revisionId, gfile.id)
