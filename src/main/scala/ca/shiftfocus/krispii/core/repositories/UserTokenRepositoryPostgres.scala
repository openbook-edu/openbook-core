...
class UserTokenRepositoryPostgres extends UserTokenRepository with PostgresRepository[UserToken] {
   ...
   * @return
   */
  override def find(userId: UUID, tokenType: String)(implicit conn: Connection, cache: ScalaCachePool): Future[\/[RepositoryError.Fail, UserToken]] = {
   ...
   * @return
   */
  override def insert(userId: UUID, nonce: String, tokenType: String)(implicit conn: Connection, cache: ScalaCachePool): Future[\/[RepositoryError.Fail, UserToken]] = {
   ...
   * @return
   */
  override def delete(userId: UUID, tokenType: String)(implicit conn: Connection, cache: ScalaCachePool): Future[\/[RepositoryError.Fail, UserToken]] = {
