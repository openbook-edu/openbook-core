...
class WorkRepositoryPostgres(
   ...
   * @return
   */
  override def list(user: User, task: Task)(implicit conn: Connection) // format: OFF
  : Future[\/[RepositoryError.Fail, Either[DocumentWork, IndexedSeq[ListWork[_ >: Int with MatchingTask.Match]]]]] = { // format: ON
    task match {
   ...
   * @return
   */
  private def listListWork(user: User, task: Task)(implicit conn: Connection) // format: OFF
  : Future[\/[RepositoryError.Fail, IndexedSeq[ListWork[_ >: Int with MatchingTask.Match]]]] = { // format: ON
    (for {
