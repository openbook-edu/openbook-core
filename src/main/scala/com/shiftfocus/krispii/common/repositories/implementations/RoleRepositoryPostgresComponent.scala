...
trait RoleRepositoryPostgresComponent extends RoleRepositoryComponent {
  ...
  private class RoleRepositoryPSQL extends RoleRepository {
    ...
    override def list(user: User): Future[IndexedSeq[Role]] = {
      val cacheString = s"role_ids:user:${user.id.string}"
      Cache.getAs[IndexedSeq[UUID]](cacheString) match {
        case Some(roleIdList) => {
          // If we found the list in cache, load each section individually
          val fRoleOptionList = Future.sequence(roleIdList.map(find))
          fRoleOptionList.map { roleOptionList =>
            Logger.debug(roleIdList.toString())
            Logger.debug(roleOptionList.toString())
            roleOptionList.map(_.get)
          }
        }
        case None => {
    ...
    override def find(id: UUID): Future[Option[Role]] = {
      val cacheString = s"roles:id:${id.string}"
      Cache.getAs[Role](cacheString) match {
        case Some(role) => Future.successful(Some(role))
        case None => {
          db.pool.sendPreparedStatement(SelectOne, Array[Any](id.bytes)).map { result =>
            result.rows.get.headOption match {
              case Some(rowData) => Some(cache(Role(rowData)))
              case None => None
            }
          }.recover {
            case exception => {
