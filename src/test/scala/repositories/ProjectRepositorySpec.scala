...
class ProjectRepositorySpec
  ...
  val projectRepository = new ProjectRepositoryPostgres(partRepository, taskRepository, componentRepository, tagRepository)
  ...
  "ProjectRepository.listByTags" should {
    inSequence {
      "find all master projects marked with a given tag" in {
        val testProjectList = TreeMap[Int, Project](
          0 -> TestValues.testProjectG,
          1 -> TestValues.testProjectH
        )

        val testSearchTagList = IndexedSeq(TestValues.testTagA.name, TestValues.testTagB.name)

        val testPartList = TreeMap(
          testProjectList(0).id.toString -> Vector(),
          testProjectList(1).id.toString -> Vector(
            TestValues.testPartI
          )
        )

        val testTagList = TreeMap(
          testProjectList(0).id.toString -> Vector(
            TestValues.testTagA,
            TestValues.testTagB
          ),
        ...
        testProjectList.foreach {
          case (key, project: Project) => {
            (partRepository.list(_: Project)(_: Connection, _: ScalaCachePool)) when (project.copy(parts = Vector()), *, *) returns (Future.successful(\/-(testPartList(project.id.toString))))
            (tagRepository.listByProjectId(_: UUID)(_: Connection)) when (project.id, *) returns (Future.successful(\/-(testTagList(project.id.toString))))
          }
        }

        val result = projectRepository.listByTags(testSearchTagList)
        val eitherProjects = Await.result(result, Duration.Inf)
        val \/-(projects) = eitherProjects
  ...
  "ProjectRepository.update" should {
    inSequence {
      "update project" in {
        ...
        project.status should be(updatedProject.status)
        project.createdAt.toString should be(testProject.createdAt.toString)
