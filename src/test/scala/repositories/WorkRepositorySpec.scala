...
class WorkRepositorySpec
  ...
  "WorkRepository.list" should {
    inSequence {
      ...
      "return RepositoryError.NoResults for task if user doesn't exist" in {
        ...
        val result = workRepository.list(testUser, testTask)
        Await.result(result, Duration.Inf) should be(-\/(RepositoryError.NoResults(s"Could not find any work for user ${testUser.id} for task ${testTask.id}")))
      }
  ...
  "WorkRepository.update" should {
    inSequence {
      ...
      "return RepositoryError.BadParam when update MatchingWork (newRevision = FALSE)" in {
        val testWork = TestValues.testMatchingWorkJ
        // Work for new values
