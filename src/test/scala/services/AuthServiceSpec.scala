...
class AuthServiceSpec
  ...
  val emailChangeRepository = stub[EmailChangeRepository]
  ...
  val tagService = stub[TagService]

  //Create a real instance of AuthService for testing
  val authService = new AuthServiceDefault(db, mailerClient, accountRepository, emailChangeRepository, organizationRepository,
    paymentLogRepository, roleRepository, sessionRepository, tagRepository, userRepository, userTokenRepository, wordRepository, tagService) {
  ...
  "AuthService.authenticate" should {
    inSequence {
      "return a user if the identifier and password combination are valid" in {
        ...
        (userRepository.find(_: String)(_: Connection)) when (testUser.username, *) returns (Future.successful(\/.right(testUser)))
        (roleRepository.list(_: User)(_: Connection)) when (testUser, *) returns (Future.successful(\/.right(testRoleList)))
      ...
      "return AuthFail if the password was wrong" in {
        ...
        val fSomeUser = authService.authenticate(testUser.username, password)
        Await.result(fSomeUser, Duration.Inf) should be(-\/(ServiceError.BadInput("The password was invalid.")))
      }
      "return RepositoryError.NoResults if the user doesn't exist" in {
        ...
        (userRepository.find(_: String)(_: Connection)) when (testUser.username, *) returns (Future.successful(-\/(RepositoryError.NoResults(""))))
        (roleRepository.list(_: User)(_: Connection)) when (testUser, *) returns (Future.successful(\/.right(testRoleList)))
  ...
  "AuthService.update" should {
    inSequence {
      "update user with unique values (ALL)" in {
        ...
        val roleUser = IndexedSeq(TestValues.testRoleA)

        (userRepository.find(_: UUID, _: Boolean)(_: Connection)) when (testUser.id, *, *) returns (Future.successful(\/-(testUser)))
        (userRepository.update(_: User)(_: Connection)) when (updatedTestUser, *) returns (Future.successful(\/-(updatedTestUser)))

        //Conflicting user
        (userRepository.find(_: String)(_: Connection)) when (updatedTestUser.email, *) returns (Future.successful(-\/(RepositoryError.NoResults(""))))
        (userRepository.find(_: String)(_: Connection)) when (updatedTestUser.username, *) returns (Future.successful(-\/(RepositoryError.NoResults(""))))
        (roleRepository.list(_: User)(_: Connection)) when (testUser, *) returns (Future.successful(\/-(roleUser)))

        val result = authService.update(testUser.id, testUser.version, Option(updatedTestUser.email),
          Option(updatedTestUser.username), Option(updatedTestUser.givenname), Option(updatedTestUser.surname),
          updatedTestUser.alias, updatedTestUser.hash, Option(updatedTestUser.isDeleted))
      ...
      "update user with unique values (only ginvenname and surname)" in {
        ...
        ////Conflicting user
        (userRepository.find(_: String)(_: Connection)) when (testUser.email, *) returns (Future.successful(\/-(testUser)))
        (userRepository.find(_: String)(_: Connection)) when (testUser.username, *) returns (Future.successful(\/-(testUser)))

        val result = authService.update(testUser.id, testUser.version, Some(updatedTestUser.email), Some(updatedTestUser.username),
          Some(updatedTestUser.givenname), Some(updatedTestUser.surname), updatedTestUser.alias, None, Some(updatedTestUser.isDeleted))
        val eitherUser = Await.result(result, Duration.Inf)
        val \/-(user) = eitherUser
      ...
      "return RepositoryError.UniqueKeyConflict if username is not unique" in {
        ...
        ////Conflicting user
        (userRepository.find(_: String)(_: Connection)) when (updatedTestUser.email, *) returns (Future.successful(-\/(RepositoryError.NoResults(""))))
        (userRepository.find(_: String)(_: Connection)) when (updatedTestUser.username, *) returns (Future.successful(\/-(conflictingUser)))
      ...
      "return RepositoryError.UniqueKeyConflict if email is not unique" in {
        ...
        ////Conflicting user
        (userRepository.find(_: String)(_: Connection)) when (updatedTestUser.email, *) returns (Future.successful(\/-(conflictingUser)))
        (userRepository.find(_: String)(_: Connection)) when (updatedTestUser.username, *) returns (Future.successful(-\/(RepositoryError.NoResults(""))))
      ...
      "return ServiceError.OfflineLockFail if version is wrong" in {
        ...
        val result = authService.updateIdentifier(testUser.id, updatedTestUser.version, Some(updatedTestUser.email), Some(updatedTestUser.username))
  ...
  "AuthService.updateRole" should {
    inSequence {
      "return ServiceError.OfflineLockFail if versions don't match" in {
        ...
        (roleRepository.find(_: UUID)(_: Connection)) when (testRole.id, *) returns (Future.successful(\/-(testRole)))
  ...
  "AuthService.activate user" in {
    ...
    val listMembers = IndexedSeq(TestValues.testOrganizationA, TestValues.testOrganizationB)
    val testActivation = TestValues.testUserToken
    (userRepository.find(_: UUID, _: Boolean)(_: Connection)) when (userId, *, *) returns (Future.successful(\/-(testUser)))
    (roleRepository.list(_: User)(_: Connection)) when (testUser, *) returns (Future.successful(\/.right(testRoleList)))
    (userTokenRepository.find(_: UUID, _: String)(_: Connection)) when (userId, "activation", *) returns (Future.successful(\/-(testActivation)))
    (roleRepository.addToUser(_: User, _: String)(_: Connection)) when (testUser, "authenticated", *) returns (Future.successful(\/.right(testRoleList)))

    (userTokenRepository.delete(_: UUID, _: String)(_: Connection)) when (userId, "activation", *) returns (Future.successful(\/-(testActivation)))
    (organizationRepository.listByMember(_: String)(_: Connection)) when (testUser.email, *) returns (Future.successful(\/-(listMembers)))
