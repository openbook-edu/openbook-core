  ...
  "ComponentService.listByProject" should {
    inSequence {
      "list components by project and user, thus listing \"enabled\" components (forceAll = TRUE)" in {
        ...
        (componentRepository.list(_: Project)(_: Connection)) when (testProject, *) returns (Future.successful(\/-(testComponentList.flatMap(_._2)(breakOut).distinct)))
      ...
      "list components by project and user, thus listing \"enabled\" components (forceAll = FALSE)" in {
        ...
        testPartList.foreach {
          case (key, part: Part) => {
            (componentRepository.list(_: Part)(_: Connection)) when (part, *) returns (Future.successful(\/-(testAllComponentList(part.id))))
          }
        }
  ...
  "ComponentService.updateAudio" should {
    inSequence {
      "return ServiceError.OfflineLockFail if versions don't match" in {
        ...
        (componentRepository.find(_: UUID)(_: Connection)) when (testComponent.id, *) returns (Future.successful(\/-(testComponent)))

        val result = componentService.updateAudio(
          testComponent.id,
          testComponent.version + 1,
          testComponent.ownerId,
          Option(testComponent.title),
          Option(testComponent.description),
          Option(testComponent.questions),
          Option(testComponent.thingsToThinkAbout),
          Option(testComponent.mediaData),
          Option(testComponent.order),
          Option(testComponent.isPrivate)
        )
      ...
      "return ServiceError.BadInput if a wrong type of component was found by ID" in {
        ...
        val result = componentService.updateAudio(
          ...
          Some(testComponent.description),
    ...
    inSequence {
      "return ServiceError.OfflineLockFail if versions don't match" in {
        ...
        val result = componentService.updateVideo(
          ...
          Some(testComponent.thingsToThinkAbout),
          Option(testComponent.mediaData),
          Option(testComponent.width),
          Option(testComponent.height),
          Option(testComponent.order),
          Option(testComponent.isPrivate)
        )
        Await.result(result, Duration.Inf) should be(-\/(ServiceError.OfflineLockFail))
      ...
      "return ServiceError.BadInput if a wrong type of component was found by ID" in {
        ...
        val result = componentService.updateVideo(
          ...
          Some(testComponent.questions),
          Some(testComponent.thingsToThinkAbout),
          Some(MediaData(Some("vimeo"), Some("19579282"))),
          Option(240),
          Option(140),
          Option(testComponent.order),
          Option(testComponent.isPrivate)
