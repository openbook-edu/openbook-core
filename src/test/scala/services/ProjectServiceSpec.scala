...
class ProjectServiceSpec
  ...
  val projectTokenRepository = stub[ProjectTokenRepository]
  ...
  val projectService = new ProjectServiceDefault(db, authService, schoolService, documentService, courseRepository,
    projectRepository, partRepository, taskRepository, workRepository, projectTokenRepository, componentRepository) {
  ...
  "ProjectService.create" should {
    inSequence {
      "return new project" in {
        ...
        (projectRepository.find(_: String)(_: Connection)) when (emptyProject.slug, *) returns (Future.successful(-\/(RepositoryError.NoResults(""))))
        (projectRepository.insert(_: Project)(_: Connection)) when (*, *) returns (Future.successful(\/-(emptyProject)))
        (partRepository.insert(_: Part)(_: Connection)) when (*, *) returns (Future.successful(\/-(emptyPart)))
        (taskRepository.insert(_: Task)(_: Connection)) when (*, *) returns (Future.successful(\/-(emptyTask)))
      ...
      "return a new project with slug + '-1' if slug is already in use" in {
        ...
        (projectRepository.find(_: String)(_: Connection)) when (emptyProject.slug, *) returns (Future.successful(\/-(testProject)))
        (projectRepository.insert(_: Project)(_: Connection)) when (*, *) returns (Future.successful(\/-(resultProject)))
        (partRepository.insert(_: Part)(_: Connection)) when (*, *) returns (Future.successful(\/-(emptyPart)))
        (taskRepository.insert(_: Task)(_: Connection)) when (*, *) returns (Future.successful(\/-(emptyTask)))
  ...
  "ProjectService.updateInfo" should {
    inSequence {
      ...
      "return RepositoryError.NoResults if project doesn't exist" in {
        ...
        (projectRepository.find(_: UUID)(_: Connection)) when (testProject.id, *) returns (Future.successful(-\/(RepositoryError.NoResults(""))))

        val result = projectService.updateInfo(testProject.id, testProject.version, Some(testProject.courseId),
          Some(testProject.name), Some(testProject.slug), Some(testProject.description), Some(testProject.longDescription),
          Some(testProject.availability), Some(testProject.enabled), Some(testProject.projectType), testProject.status, Some(testProject.lastTaskId))
  ...
  "ProjectService.updateSlug" should {
    inSequence {
      "update slug" in {
        ...
        (projectRepository.find(_: UUID)(_: Connection)) when (testProject.id, *) returns (Future.successful(\/-(testProject)))
        (projectRepository.find(_: String)(_: Connection)) when (updatedProject.slug, *) returns (Future.successful(-\/(RepositoryError.NoResults(""))))
        (projectRepository.update(_: Project)(_: Connection)) when (updatedProject, *) returns (Future.successful(\/-(updatedProject)))
      ...
      "return updated project with slug + '-1'  if slug is already in use" in {
        ...
        (projectRepository.find(_: UUID)(_: Connection)) when (testProject.id, *) returns (Future.successful(\/-(testProject)))
        (projectRepository.find(_: String)(_: Connection)) when (updatedProject.slug, *) returns (Future.successful(\/-(testProject)))
        (projectRepository.update(_: Project)(_: Connection)) when (updatedProject, *) returns (Future.successful(\/-(resultProject)))
  ...
  "ProjectService.delete" should {
    inSequence {
      "return ServiceError.OfflineLockFail if versions don't match" in {
        ...
        (projectRepository.find(_: UUID, _: Boolean)(_: Connection)) when (testProject.id, false, *) returns (Future.successful(\/-(testProject)))
      ...
      "return RepositoryError.NoResults if project doesn't exist" in {
        ...
        (projectRepository.find(_: UUID, _: Boolean)(_: Connection)) when (testProject.id, false, *) returns (Future.successful(-\/(RepositoryError.NoResults(""))))
  ...
  "ProjectService.createPart" should {
    inSequence {
      ...
      "create new part and update existing parts if new part position = min position" in {
        ...
        (projectRepository.find(_: UUID, _: Boolean)(_: Connection)) when (testProject.id, false, *) returns (Future.successful(\/-(noPartsProject)))
        (partRepository.list(_: Project, _: Boolean, _: Boolean)(_: Connection)) when (noPartsProject, false, false, *) returns (Future.successful(\/-(testPartList)))

        for (i <- testPartList.indices) {
          (partRepository.update(_: Part)(_: Connection)) when (testPartList(i).copy(position = i + 2), *) returns (Future.successful(\/-(testPartList(i).copy(position = i + 2))))
        }

        (partRepository.insert(_: Part)(_: Connection)) when (newPart.copy(position = 1), *) returns (Future.successful(\/-(newPart.copy(position = 1))))
      ...
      "create new part and update position of all element if new part position = max position" in {
        ...
        for (i <- testPartList.indices) {
          if (i + 1 != testPartList.length)
            (partRepository.update(_: Part)(_: Connection)) when (testPartList(i).copy(position = i + 1), *) returns (Future.successful(\/-(testPartList(i).copy(position = i + 1))))
        }

        (partRepository.update(_: Part)(_: Connection)) when (testPartList.last.copy(position = testPartList.length + 1), *) returns (Future.successful(\/-(testPartList.last.copy(position = testPartList.length + 1))))

        (partRepository.insert(_: Part)(_: Connection)) when (newPart.copy(position = testPartList.length), *) returns (Future.successful(\/-(newPart.copy(position = testPartList.length))))
      ...
      "create new part and insert between elements and update positions of all elements even if they are unsorted " in {
        ...
        //mock partA
        (partRepository.update(_: Part)(_: Connection)) when (testPartList(0).copy(position = 3), *) returns (Future.successful(\/-(testPartList(0).copy(position = 3))))
        //mock PartB
        (partRepository.update(_: Part)(_: Connection)) when (testPartList(1).copy(position = 1), *) returns (Future.successful(\/-(testPartList(1).copy(position = 1))))
        //mock partC
        (partRepository.update(_: Part)(_: Connection)) when (testPartList(2).copy(position = 4), *) returns (Future.successful(\/-(testPartList(2).copy(position = 4))))
        //mock newPart
        (partRepository.insert(_: Part)(_: Connection)) when (newPart.copy(position = 2), *) returns (Future.successful(\/-(newPart.copy(position = 2))))
      ...
      "create new part and give it position of last element + 1 if new part position > max position" in {
        ...
        (partRepository.insert(_: Part)(_: Connection)) when (newPart.copy(position = testPartList.length + 1), *) returns (Future.successful(\/-(newPart.copy(position = testPartList.length + 1))))
      ...
      "create new part and give it position 1 if partList is not empty and partList positions start with 0 and we assign position 1 to newPart" in {
        ...
        (partRepository.insert(_: Part)(_: Connection)) when (newPart, *) returns (Future.successful(\/-(newPart)))
  ...
  "ProjectService.updatePart" + (Console.YELLOW + Console.BOLD + " - These tests include also ProjectService.movePart method verification" + Console.RESET) should {
    inSequence {
      "update part and update existing parts if new part position = min position" in {
        ...
        (partRepository.find(_: UUID)(_: Connection)) when (testPartList(1).id, *) returns (Future.successful(\/-(testPartList(1))))
        (projectRepository.find(_: UUID, _: Boolean)(_: Connection)) when (updatedPart.projectId, false, *) returns (Future.successful(\/-(noPartsProject)))
        (partRepository.list(_: Project, _: Boolean, _: Boolean)(_: Connection)) when (noPartsProject, false, false, *) returns (Future.successful(\/-(testPartList)))

        for (i <- filteredPartList.indices) {
          (partRepository.update(_: Part)(_: Connection)) when (filteredPartList(i).copy(position = i + 2), *) returns (Future.successful(\/-(filteredPartList(i).copy(position = i + 2))))
        }

        (partRepository.update(_: Part)(_: Connection)) when (testPartList(1).copy(position = 1), *) returns (Future.successful(\/-(testPartList(1).copy(position = 1))))
        (partRepository.update(_: Part)(_: Connection)) when (updatedPart.copy(position = 1), *) returns (Future.successful(\/-(updatedPart.copy(position = 1))))
      ...
      "update part and preserve position if newPosition != oldPosition, but position is next part position (partList positions are unsorted)" in {
        ...
        (partRepository.find(_: UUID)(_: Connection)) when (testPartList(0).id, *) returns (Future.successful(\/-(testPartList(0))))
        (projectRepository.find(_: UUID, _: Boolean)(_: Connection)) when (updatedPart.projectId, false, *) returns (Future.successful(\/-(noPartsProject)))
        (partRepository.list(_: Project, _: Boolean, _: Boolean)(_: Connection)) when (noPartsProject, false, false, *) returns (Future.successful(\/-(testPartList)))

        //partA
        (partRepository.update(_: Part)(_: Connection)) when (testPartList(0).copy(position = 2), *) returns (Future.successful(\/-(testPartList(0).copy(position = 2))))
        (partRepository.update(_: Part)(_: Connection)) when (updatedPart.copy(position = 2), *) returns (Future.successful(\/-(updatedPart.copy(position = 2))))
        //partB
        (partRepository.update(_: Part)(_: Connection)) when (testPartList(1).copy(position = 1), *) returns (Future.successful(\/-(testPartList(1).copy(position = 1))))
        //partC
        (partRepository.update(_: Part)(_: Connection)) when (testPartList(2).copy(position = 3), *) returns (Future.successful(\/-(testPartList(2).copy(position = 3))))
        //partE
        (partRepository.update(_: Part)(_: Connection)) when (testPartList(3).copy(position = 4), *) returns (Future.successful(\/-(testPartList(3).copy(position = 4))))
      ...
      "update part and update position if newPosition != oldPosition and position is next part + 1 position" in {
        ...
        //partA
        (partRepository.update(_: Part)(_: Connection)) when (testPartList(0).copy(position = 3), *) returns (Future.successful(\/-(testPartList(0).copy(position = 3))))
        (partRepository.update(_: Part)(_: Connection)) when (updatedPart.copy(position = 3), *) returns (Future.successful(\/-(updatedPart.copy(position = 3))))
        //partB
        (partRepository.update(_: Part)(_: Connection)) when (testPartList(1).copy(position = 1), *) returns (Future.successful(\/-(testPartList(1).copy(position = 1))))
        //partC
        (partRepository.update(_: Part)(_: Connection)) when (testPartList(2).copy(position = 2), *) returns (Future.successful(\/-(testPartList(2).copy(position = 2))))
        //partE
        (partRepository.update(_: Part)(_: Connection)) when (testPartList(3).copy(position = 4), *) returns (Future.successful(\/-(testPartList(3).copy(position = 4))))
      ...
      "update part if newPosition == oldPosition" in {
        ...
        //partA
        (partRepository.update(_: Part)(_: Connection)) when (testPartList(0).copy(position = 1), *) returns (Future.successful(\/-(testPartList(0).copy(position = 1))))
        (partRepository.update(_: Part)(_: Connection)) when (updatedPart.copy(position = 1), *) returns (Future.successful(\/-(updatedPart.copy(position = 1))))
        //partB
        (partRepository.update(_: Part)(_: Connection)) when (testPartList(1).copy(position = 2), *) returns (Future.successful(\/-(testPartList(1).copy(position = 2))))
        //partC
        (partRepository.update(_: Part)(_: Connection)) when (testPartList(2).copy(position = 3), *) returns (Future.successful(\/-(testPartList(2).copy(position = 3))))
      ...
      "update part and put it as first element if newPosition == 1 and part list has positions that start from 0" in {
        ...
        (partRepository.find(_: UUID)(_: Connection)) when (testPartList(2).id, *) returns (Future.successful(\/-(testPartList(2))))
        (projectRepository.find(_: UUID, _: Boolean)(_: Connection)) when (updatedPart.projectId, false, *) returns (Future.successful(\/-(noPartsProject)))
        (partRepository.list(_: Project, _: Boolean, _: Boolean)(_: Connection)) when (noPartsProject, false, false, *) returns (Future.successful(\/-(testPartList)))

        //partA
        (partRepository.update(_: Part)(_: Connection)) when (testPartList(1).copy(position = 3), *) returns (Future.successful(\/-(testPartList(1).copy(position = 3))))
        //partB
        (partRepository.update(_: Part)(_: Connection)) when (testPartList(2).copy(position = updatedPart.position), *) returns (Future.successful(\/-(testPartList(2).copy(position = updatedPart.position))))
        (partRepository.update(_: Part)(_: Connection)) when (updatedPart, *) returns (Future.successful(\/-(updatedPart)))
        //partC
        (partRepository.update(_: Part)(_: Connection)) when (testPartList(0).copy(position = 2), *) returns (Future.successful(\/-(testPartList(0).copy(position = 2))))
      ...
      "update only one indicated part if newPosition == oldPosition, and position of other parts is correct (starts with 1)" in {
        ...
        (partRepository.find(_: UUID)(_: Connection)) when (updatedPart.id, *) returns (Future.successful(\/-(testPartList(1))))
        (projectRepository.find(_: UUID, _: Boolean)(_: Connection)) when (updatedPart.projectId, false, *) returns (Future.successful(\/-(noPartsProject)))
        (partRepository.list(_: Project, _: Boolean, _: Boolean)(_: Connection)) when (noPartsProject, false, false, *) returns (Future.successful(\/-(testPartList)))
      ...
      "return ServiceError.OfflineLockFail if versions don't match" in {
        ...
        (partRepository.find(_: UUID)(_: Connection)) when (updatedPart.id, *) returns (Future.successful(\/-(updatedPart)))
      ...
      "return ServiceError.BusinessLogicFail if part list is empty and we change position" in {
        ...
        (partRepository.find(_: UUID)(_: Connection)) when (updatedPart.id, *) returns (Future.successful(\/-(testPartList(1))))
        (projectRepository.find(_: UUID, _: Boolean)(_: Connection)) when (updatedPart.projectId, false, *) returns (Future.successful(\/-(noPartsProject)))
        (partRepository.list(_: Project, _: Boolean, _: Boolean)(_: Connection)) when (noPartsProject, false, false, *) returns (Future.successful(\/-(IndexedSeq.empty[Part])))
      ...
      "return RepositoryError.NoResults if project doesn't exist and we change position" in {
        ...
        (partRepository.find(_: UUID)(_: Connection)) when (updatedPart.id, *) returns (Future.successful(\/-(testPartList(1))))
        (projectRepository.find(_: UUID, _: Boolean)(_: Connection)) when (updatedPart.projectId, false, *) returns (Future.successful(-\/(RepositoryError.NoResults(""))))
        (partRepository.list(_: Project, _: Boolean, _: Boolean)(_: Connection)) when (noPartsProject, false, false, *) returns (Future.successful(\/-(testPartList)))
      ...
      "return RepositoryError.NoResults if part doesn't exist" in {
        ...
        (partRepository.find(_: UUID)(_: Connection)) when (updatedPart.id, *) returns (Future.successful(-\/(RepositoryError.NoResults(""))))
        (projectRepository.find(_: UUID, _: Boolean)(_: Connection)) when (updatedPart.projectId, false, *) returns (Future.successful(\/-(noPartsProject)))
        (partRepository.list(_: Project, _: Boolean, _: Boolean)(_: Connection)) when (noPartsProject, false, false, *) returns (Future.successful(\/-(testPartList)))
  ...
  "ProjectService.deletePart" should {
    inSequence {
      ...
      "delete part between two parts" in {
        ...
        (partRepository.find(_: UUID)(_: Connection)) when (deletedPart.id, *) returns (Future.successful(\/-(deletedPart)))
        (projectRepository.find(_: UUID, _: Boolean)(_: Connection)) when (testProject.id, false, *) returns (Future.successful(\/-(noPartsProject)))
        (partRepository.list(_: Project, _: Boolean, _: Boolean)(_: Connection)) when (noPartsProject, false, false, *) returns (Future.successful(\/-(testPartList)))
        ...
        (taskRepository.delete(_: Part)(_: Connection)) when (deletedPart, *) returns (Future.successful(\/-(IndexedSeq())))
        (partRepository.delete(_: Part)(_: Connection)) when (deletedPart, *) returns (Future.successful(\/-(deletedPart)))
      ...
      "delete part in the beginning" in {
        ...
        (partRepository.find(_: UUID)(_: Connection)) when (deletedPart.id, *) returns (Future.successful(\/-(deletedPart)))
        (projectRepository.find(_: UUID, _: Boolean)(_: Connection)) when (deletedPart.projectId, false, *) returns (Future.successful(\/-(noPartsProject)))
        (partRepository.list(_: Project, _: Boolean, _: Boolean)(_: Connection)) when (noPartsProject, false, false, *) returns (Future.successful(\/-(testPartList)))
      ...
      "return RepositoryError.NoResults if project doesn't exist" in {
        ...
        (partRepository.find(_: UUID)(_: Connection)) when (deletedPart.id, *) returns (Future.successful(\/-(deletedPart)))
        (projectRepository.find(_: UUID, _: Boolean)(_: Connection)) when (deletedPart.projectId, false, *) returns (Future.successful(-\/(RepositoryError.NoResults(""))))
        (partRepository.list(_: Project, _: Boolean, _: Boolean)(_: Connection)) when (noPartsProject, false, false, *) returns (Future.successful(\/-(testPartList)))
      ...
      "return RepositoryError.NoResults if part doesn't exist" in {
        ...
        (partRepository.find(_: UUID)(_: Connection)) when (deletedPart.id, *) returns (Future.successful(-\/(RepositoryError.NoResults(""))))
        (projectRepository.find(_: UUID, _: Boolean)(_: Connection)) when (deletedPart.projectId, false, *) returns (Future.successful(\/-(noPartsProject)))
        (partRepository.list(_: Project, _: Boolean, _: Boolean)(_: Connection)) when (noPartsProject, false, false, *) returns (Future.successful(\/-(testPartList)))
  ...
  "ProjectService.togglePart" should {
    inSequence {
      "enable a disabled part" in {
        ...
        (partRepository.find(_: UUID)(_: Connection)) when (testPart.id, *) returns (Future.successful(\/-(testPart)))
        (partRepository.update(_: Part)(_: Connection)) when (testPart.copy(enabled = !testPart.enabled), *) returns (Future.successful(\/-(testPart.copy(enabled = !testPart.enabled))))
      ...
      "return RepositoryError.NoResults if part doesn't exist" in {
        ...
        (partRepository.find(_: UUID)(_: Connection)) when (testPart.id, *) returns (Future.successful(-\/(RepositoryError.NoResults(""))))
        (partRepository.update(_: Part)(_: Connection)) when (testPart.copy(enabled = !testPart.enabled), *) returns (Future.successful(\/-(testPart.copy(enabled = !testPart.enabled))))
  ...
  "ProjectService.createTask" should {
    inSequence {
      "create new task and update existing tasks if new task position = min position" in {
        ...
        for (i <- testTaskList.indices) {
          testTaskList(i) match {
            case task: DocumentTask => (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (task.copy(position = i + 2), *, *) returns (Future.successful(\/-(task.copy(position = i + 2))))
            case task: QuestionTask => (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (task.copy(position = i + 2), *, *) returns (Future.successful(\/-(task.copy(position = i + 2))))
          }
        }

        (taskRepository.insert(_: Task)(_: Connection)) when (newTask.copy(position = 1), *) returns (Future.successful(\/-(newTask.copy(position = 1))))
      ...
      "create new task and update position of all elements if new task position = max position" in {
        ...
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (testTaskList(0).asInstanceOf[DocumentTask].copy(position = 1), *, *) returns (Future.successful(\/-(testTaskList(0).asInstanceOf[DocumentTask].copy(position = 1))))
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (testTaskList(1).asInstanceOf[QuestionTask].copy(position = 2), *, *) returns (Future.successful(\/-(testTaskList(1).asInstanceOf[QuestionTask].copy(position = 2))))
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (testTaskList(2).asInstanceOf[QuestionTask].copy(position = testTaskList.length + 1), *, *) returns (Future.successful(\/-(testTaskList(2).asInstanceOf[QuestionTask].copy(position = testTaskList.length + 1))))
        (taskRepository.insert(_: Task)(_: Connection)) when (newTask.copy(position = testTaskList.length), *) returns (Future.successful(\/-(newTask.copy(position = testTaskList.length))))
      ...
      "create new task and insert between elements and update positions of all elements even if they are unsorted " in {
        ...
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (testTaskList(0).asInstanceOf[DocumentTask].copy(position = 3), *, *) returns (Future.successful(\/-(testTaskList(0).asInstanceOf[DocumentTask].copy(position = 3))))
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (testTaskList(1).asInstanceOf[QuestionTask].copy(position = 1), *, *) returns (Future.successful(\/-(testTaskList(1).asInstanceOf[QuestionTask].copy(position = 1))))
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (testTaskList(2).asInstanceOf[QuestionTask].copy(position = 4), *, *) returns (Future.successful(\/-(testTaskList(2).asInstanceOf[QuestionTask].copy(position = 4))))
        (taskRepository.insert(_: Task)(_: Connection)) when (newTask.copy(position = 2), *) returns (Future.successful(\/-(newTask.copy(position = 2))))
      ...
      "create new task and give it position of last element + 1 if new task position > max position" in {
        ...
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (testTaskList(0).asInstanceOf[DocumentTask].copy(position = 1), *, *) returns (Future.successful(\/-(testTaskList(0).asInstanceOf[DocumentTask].copy(position = 1))))
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (testTaskList(1).asInstanceOf[QuestionTask].copy(position = 2), *, *) returns (Future.successful(\/-(testTaskList(1).asInstanceOf[QuestionTask].copy(position = 2))))
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (testTaskList(2).asInstanceOf[QuestionTask].copy(position = 3), *, *) returns (Future.successful(\/-(testTaskList(2).asInstanceOf[QuestionTask].copy(position = 3))))
        (taskRepository.insert(_: Task)(_: Connection)) when (newTask.copy(position = 4), *) returns (Future.successful(\/-(newTask.copy(position = 4))))
      ...
      "create new task and give it position 1 if taskList is not empty and taskList positions start with 0 and we assign position 1 to newTask" in {
        ...
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (testTaskList(0).asInstanceOf[DocumentTask].copy(position = 2), *, *) returns (Future.successful(\/-(testTaskList(0).asInstanceOf[DocumentTask].copy(position = 2))))
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (testTaskList(1).asInstanceOf[QuestionTask].copy(position = 3), *, *) returns (Future.successful(\/-(testTaskList(1).asInstanceOf[QuestionTask].copy(position = 3))))
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (testTaskList(2).asInstanceOf[QuestionTask].copy(position = 4), *, *) returns (Future.successful(\/-(testTaskList(2).asInstanceOf[QuestionTask].copy(position = 4))))
        (taskRepository.insert(_: Task)(_: Connection)) when (newTask.copy(position = 1), *) returns (Future.successful(\/-(newTask.copy(position = 1))))
      ...
      "create new task and add it to the end of list and do not update taskList positions if they are correct (1, 2, ...)" in {
        ...
        (taskRepository.insert(_: Task)(_: Connection)) when (newTask.copy(position = testTaskList.length + 1), *) returns (Future.successful(\/-(newTask.copy(position = testTaskList.length + 1))))
  ...
  "ProjectService.updateDocumentTask (LongAnswerTask)" should {
    inSequence {
      "update DocumentTask and update existing tasks if new part position = min position within same part" in {
        ...
        val commonTaskArgs = projectService.CommonTaskArgs(
          ...
          instructions = Some(updatedTask.settings.instructions),
          tagline = Some(updatedTask.settings.tagline),
          ...
          allowGfile = Some(updatedTask.settings.allowGfile),
          partId = Some(updatedTask.partId),
          maxGrade = Some(updatedTask.maxGrade),
          mediaData = Some(updatedTask.settings.mediaData),
          layout = Some(updatedTask.settings.layout)
        )

        (taskRepository.find(_: UUID)(_: Connection)) when (originTask.id, *) returns (Future.successful(\/-(originTask)))
        (partRepository.find(_: UUID)(_: Connection)) when (testPart.id, *) returns (Future.successful(\/-(testPart)))
        (taskRepository.find(_: UUID)(_: Connection)) when (commonTaskArgs.taskId, *) returns (Future.successful(\/-(originTask.copy(position = 1))))

        //Move
        //task A
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (originTask.copy(position = 1), *, *) returns (Future.successful(\/-(originTask.copy(position = 1))))
        //task B
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (testTaskList(1).asInstanceOf[QuestionTask].copy(position = 2), *, *) returns (Future.successful(\/-(testTaskList(1).asInstanceOf[QuestionTask].copy(position = 2))))
        //task C
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (testTaskList(2).asInstanceOf[QuestionTask].copy(position = 3), *, *) returns (Future.successful(\/-(testTaskList(2).asInstanceOf[QuestionTask].copy(position = 3))))
        ...
        //update task A
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (updatedTask.copy(position = 1), *, *) returns (Future.successful(\/-(updatedTask.copy(position = 1))))
      ...
      "return ServiceError.BadInput if task has wrong type" in {
        ...
        (taskRepository.find(_: UUID)(_: Connection)) when (originTask.id, *) returns (Future.successful(\/-(testTaskList(1))))
  ...
  "ProjectService.updateQuestionTask (ShortAnswerTask)" should {
    inSequence {
      "update QuestionTask and update existing tasks if new part position = min position (within same part)" in {
        ...
        (taskRepository.find(_: UUID)(_: Connection)) when (testTaskList(1).id, *) returns (Future.successful(\/-(testTaskList(1))))
        (partRepository.find(_: UUID)(_: Connection)) when (testPart.id, *) returns (Future.successful(\/-(testPart)))

        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (testTaskList(0).asInstanceOf[DocumentTask].copy(position = 2), *, *) returns (Future.successful(\/-(testTaskList(0).asInstanceOf[DocumentTask].copy(position = 2, version = testTaskList(0).version + 1))))
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (testTaskList(1).asInstanceOf[QuestionTask].copy(position = 1), *, *) returns (Future.successful(\/-(testTaskList(1).asInstanceOf[QuestionTask].copy(position = 1, version = testTaskList(1).version + 1))))
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (testTaskList(2).asInstanceOf[QuestionTask].copy(position = 3), *, *) returns (Future.successful(\/-(testTaskList(2).asInstanceOf[QuestionTask].copy(position = 3, version = testTaskList(2).version + 1))))
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (updatedTask.copy(position = 1, version = updatedTask.version + 1), *, *) returns (Future.successful(\/-(updatedTask.copy(position = 1, version = updatedTask.version + 2))))
      ...
      "return ServiceError.BadInput if task has wrong type" in {
        ...
        (taskRepository.find(_: UUID)(_: Connection)) when (originTask.id, *) returns (Future.successful(\/-(testTaskList(0))))
  ...
  "ProjectService.updateQuestionTask (MultipleChoiceTask)" should {
    inSequence {
      "update QuestionTask and update existing tasks if new part position = min position within same part" in {
        ...
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (testTaskList(0).asInstanceOf[DocumentTask].copy(position = 2), *, *) returns (Future.successful(\/-(testTaskList(0).asInstanceOf[DocumentTask].copy(position = 2, version = testTaskList(0).version + 1))))
        //task B
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (testTaskList(1).asInstanceOf[QuestionTask].copy(position = 3), *, *) returns (Future.successful(\/-(testTaskList(1).asInstanceOf[QuestionTask].copy(position = 3, version = testTaskList(1).version + 1))))
        //task C
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (originTask.copy(position = 1), *, *) returns (Future.successful(\/-(originTask.copy(position = 1, version = originTask.version + 1))))
  ...
  "ProjectService.updateQuestionTask (OrderingTask)" should {
    inSequence {
      "update OrderingTask and update existing tasks if new part position = min position within same part" in {
        ...
        //Move
        //task D
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (testTaskList(0).asInstanceOf[QuestionTask].copy(position = 2), *, *) returns (Future.successful(\/-(testTaskList(0).asInstanceOf[QuestionTask].copy(position = 2, version = testTaskList(0).version + 1))))
        //task K
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (testTaskList(1).asInstanceOf[QuestionTask].copy(position = 3), *, *) returns (Future.successful(\/-(testTaskList(1).asInstanceOf[QuestionTask].copy(position = 3, version = testTaskList(1).version + 1))))
        //task L
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (originTask.copy(position = 1), *, *) returns (Future.successful(\/-(originTask.copy(position = 1, version = originTask.version + 1))))
      ...
      "update OrderingTask and update notesTitle, responseTitle to None (aka erase)" in {
        ...
        //Move
        //task D
        (taskRepository.update(_: Task)(_: Connection)) when (testTaskList(0).asInstanceOf[QuestionTask].copy(position = 2), *) returns (Future.successful(\/-(testTaskList(0).asInstanceOf[QuestionTask].copy(position = 2, version = testTaskList(0).version + 1))))
        //task K
        (taskRepository.update(_: Task)(_: Connection)) when (testTaskList(1).asInstanceOf[QuestionTask].copy(position = 3), *) returns (Future.successful(\/-(testTaskList(1).asInstanceOf[QuestionTask].copy(position = 3, version = testTaskList(1).version + 1))))
        //task L
        (taskRepository.update(_: Task)(_: Connection)) when (originTask.copy(position = 1), *) returns (Future.successful(\/-(originTask.copy(position = 1, version = originTask.version + 1))))

        //update task L
        (taskRepository.update(_: Task)(_: Connection)) when (updatedTask.copy(position = 1, version = updatedTask.version + 1), *) returns (Future.successful(\/-(updatedTask.copy(position = 1, version = updatedTask.version + 2))))
      ...
      "return ServiceError.BadInput if task has wrong type" in {
        ...
        (taskRepository.find(_: UUID)(_: Connection)) when (originTask.id, *) returns (Future.successful(\/-(wrongTypeTask)))
  ...
  "ProjectService.moveTask" should {
    inSequence {
      ...
      "move QuestionTask and update old part tasks positions and new part tasks positions if task new position = min position within another part" in {
        ...
        (taskRepository.find(_: UUID)(_: Connection)) when (originTask.id, *) returns (Future.successful(\/-(originTask)))
        (partRepository.find(_: UUID)(_: Connection)) when (testPart.id, *) returns (Future.successful(\/-(testPart)))
        (partRepository.find(_: UUID)(_: Connection)) when (testAnotherPart.id, *) returns (Future.successful(\/-(testAnotherPart)))

        //Move part A
        //task A
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (testTaskList(0).asInstanceOf[DocumentTask].copy(position = 1), *, *) returns (Future.successful(\/-(testTaskList(0).asInstanceOf[DocumentTask].copy(position = 1, version = testTaskList(0).version + 1))))
        //task B
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (testTaskList(1).asInstanceOf[QuestionTask].copy(position = 2), *, *) returns (Future.successful(\/-(testTaskList(1).asInstanceOf[QuestionTask].copy(position = 2, version = testTaskList(1).version + 1))))

        //Move part B
        //task C
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (updatedTask.copy(position = 1), *, *) returns (Future.successful(\/-(updatedTask.copy(position = 1, version = updatedTask.version + 1))))
        //task D
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (testAnotherTaskList(0).asInstanceOf[QuestionTask].copy(position = 2), *, *) returns (Future.successful(\/-(testAnotherTaskList(0).asInstanceOf[QuestionTask].copy(position = 2, version = testAnotherTaskList(0).version + 1))))
        //task K
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (testAnotherTaskList(1).asInstanceOf[QuestionTask].copy(position = 3), *, *) returns (Future.successful(\/-(testAnotherTaskList(1).asInstanceOf[QuestionTask].copy(position = 3, version = testAnotherTaskList(1).version + 1))))
      ...
      "NOT move QuestionTask if newPosition != oldPosition, but position is next task position (taskList positions are unsorted) within same part" in {
        ...
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (testTaskList(1).asInstanceOf[QuestionTask].copy(position = 3), *, *) returns (Future.successful(\/-(testTaskList(1).asInstanceOf[QuestionTask].copy(position = 3))))
        //task C
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (testTaskList(2).asInstanceOf[QuestionTask].copy(position = 2), *, *) returns (Future.successful(\/-(testTaskList(2).asInstanceOf[QuestionTask].copy(position = 3))))
      ...
      "move task to another part and give it max position - 1, if newPosition != oldPosition and position = max position within another part" in {
        ...
        //Move part B
        //task C
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (updatedTask.copy(position = 2), *, *) returns (Future.successful(\/-(updatedTask.copy(position = 2))))
        //task D
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (testAnotherTaskList(0).asInstanceOf[QuestionTask].copy(position = 1), *, *) returns (Future.successful(\/-(testAnotherTaskList(0).asInstanceOf[QuestionTask].copy(position = 1))))
        //task K
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (testAnotherTaskList(1).asInstanceOf[QuestionTask].copy(position = 3), *, *) returns (Future.successful(\/-(testAnotherTaskList(1).asInstanceOf[QuestionTask].copy(position = 3))))
      ...
      "move task to the last position if newPosition != oldPosition and position > max position within same part" in {
        ...
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (testTaskList(1).asInstanceOf[QuestionTask].copy(position = 1), *, *) returns (Future.successful(\/-(testTaskList(1).asInstanceOf[QuestionTask].copy(position = 1))))
        //task C
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (testTaskList(2).asInstanceOf[QuestionTask].copy(position = 2), *, *) returns (Future.successful(\/-(testTaskList(2).asInstanceOf[QuestionTask].copy(position = 2))))
      ...
      "move task to the last position in another part if newPosition != oldPosition and position > max position within another part" in {
        ...
        //Move part B
        //task C
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (updatedTask.copy(position = 3), *, *) returns (Future.successful(\/-(updatedTask.copy(position = 3))))
        //task D
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (testAnotherTaskList(0).asInstanceOf[QuestionTask].copy(position = 1), *, *) returns (Future.successful(\/-(testAnotherTaskList(0).asInstanceOf[QuestionTask].copy(position = 1))))
        //task K
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (testAnotherTaskList(1).asInstanceOf[QuestionTask].copy(position = 2), *, *) returns (Future.successful(\/-(testAnotherTaskList(1).asInstanceOf[QuestionTask].copy(position = 2))))
      ...
      "move task and put it as first element if newPosition == 1 and task list has positions that start from 0 within same part" in {
        ...
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (testTaskList(1).asInstanceOf[QuestionTask].copy(position = 3), *, *) returns (Future.successful(\/-(testTaskList(1).asInstanceOf[QuestionTask].copy(position = 3))))
        //task C
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (testTaskList(2).asInstanceOf[QuestionTask].copy(position = 1), *, *) returns (Future.successful(\/-(testTaskList(2).asInstanceOf[QuestionTask].copy(position = 1))))
      ...
      "throw an Exception if current part doesn't have tasks" in {
        ...
        (taskRepository.find(_: UUID)(_: Connection)) when (originTask.id, *) returns (Future.successful(\/-(originTask)))
        (partRepository.find(_: UUID)(_: Connection)) when (testPart.id, *) returns (Future.successful(\/-(testPart)))
        (partRepository.find(_: UUID)(_: Connection)) when (anotherTestPart.id, *) returns (Future.successful(\/-(anotherTestPart)))
      ...
      "return SRepositoryError.NoResults if another part doesn't exist" in {
        ...
        (taskRepository.find(_: UUID)(_: Connection)) when (originTask.id, *) returns (Future.successful(\/-(originTask)))
        (partRepository.find(_: UUID)(_: Connection)) when (testPart.id, *) returns (Future.successful(\/-(testPart)))
        (partRepository.find(_: UUID)(_: Connection)) when (testAnotherPart.id, *) returns (Future.successful(-\/(RepositoryError.NoResults(""))))
      ...
      "return RepositoryError.NoResults if task doesn't exist" in {
        ...
        (taskRepository.find(_: UUID)(_: Connection)) when (originTask.id, *) returns (Future.successful(-\/(RepositoryError.NoResults(""))))
        (partRepository.find(_: UUID)(_: Connection)) when (testPart.id, *) returns (Future.successful(\/-(testPart)))
  ...
  "ProjectService.deleteTask" should {
    inSequence {
      "delete task between two tasks" in {
        ...
        (taskRepository.find(_: UUID)(_: Connection)) when (deletedTask.id, *) returns (Future.successful(\/-(deletedTask)))
        (partRepository.find(_: UUID)(_: Connection)) when (testPart.id, *) returns (Future.successful(\/-(testPart)))
        ...
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (testTaskList(0).asInstanceOf[DocumentTask].copy(position = 1), *, *) returns (Future.successful(\/-(testTaskList(0).asInstanceOf[DocumentTask].copy(position = 1))))
        //task B
        (taskRepository.delete(_: Task)(_: Connection)) when (deletedTask, *) returns (Future.successful(\/-(deletedTask)))
        //task C
        (taskRepository.update(_: Task, _: Option[UUID])(_: Connection)) when (testTaskList(2).asInstanceOf[QuestionTask].copy(position = 2), *, *) returns (Future.successful(\/-(testTaskList(2).asInstanceOf[QuestionTask].copy(position = 2))))
      ...
      "return RepositoryError.NoResults if task doesn't exist" in {
        ...
        (taskRepository.find(_: UUID)(_: Connection)) when (deletedTask.id, *) returns (Future.successful(-\/(RepositoryError.NoResults(""))))
  ...
  "ProjectService.copyMasterProject" should {
    inSequence {
      "copy one master project into a group" in {
        ...
        (projectRepository.cloneProject(_: UUID, _: UUID)(_: Connection)) when (TestValues.testProjectH.id, TestValues.testCourseH.id, *) returns (Future.successful(\/-(expectedProject)))
        (projectRepository.insert(_: Project)(_: Connection)) when (expectedProject, *) returns (Future.successful(\/-(expectedProject)))
        (projectRepository.cloneProjectParts(_: UUID, _: UUID, _: UUID)(_: Connection)) when (TestValues.testProjectH.id, TestValues.testUserH.id, expectedProject.id, *) returns (Future.successful(\/-(IndexedSeq(TestValues.testPartI))))
        (partRepository.insert(_: Part)(_: Connection)) when (*, *) returns (Future.successful(\/-(TestValues.testPartC)))
        (projectRepository.cloneProjectComponents(_: UUID, _: UUID, _: Boolean)(_: Connection)) when (TestValues.testProjectH.id, TestValues.testUserH.id, *, *) returns (Future.successful(\/-(IndexedSeq(TestValues.testRubricComponentK, TestValues.testGenericHTMLComponentM))))
        ...
        (componentRepository.addToPart(_: Component, _: Part)(_: Connection)) when (*, *, *) returns (Future.successful(\/-(Unit)))
        (componentRepository.addToPart(_: Component, _: Part)(_: Connection)) when (*, *, *) returns (Future.successful(\/-(Unit)))
        (projectRepository.find(_: UUID)(_: Connection)) when (TestValues.testProjectH.id, *) returns (Future.successful(\/-(TestValues.testProjectH)))
        (courseRepository.find(_: UUID)(_: Connection)) when (TestValues.testProjectH.courseId, *) returns (Future.successful(\/-(TestValues.testCourseH)))

        val result = projectService.copyProject(TestValues.testProjectH.id, TestValues.testCourseH.id, TestValues.testUserH.id)
