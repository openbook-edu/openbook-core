...
import ca.shiftfocus.krispii.core.services.{AuthService, AuthServiceDefault, SchoolServiceDefault}
...
class SchoolServiceSpec
  ...
  val schoolService = new SchoolServiceDefault(db, authService, userRepository, courseRepository, chatRepository,
    lastSeenRepository, wordRepository, linkRepository, limitRepository, tagRepository, organizationRepository) {
  ...
  "SchoolService.updateCourse" should {
    inSequence {
      "update group" in {
        ...
        (courseRepository.find(_: UUID)(_: Connection)) when (testCourse.id, *) returns (Future.successful(\/-(testCourse)))
        (authService.find(_: UUID)) when (newTeacher.id) returns (Future.successful(\/-(newTeacher.copy(roles = testRoles))))
        (courseRepository.update(_: Course)(_: Connection)) when (updatedCourse, *) returns (Future.successful(\/-(updatedCourse)))
        ...
        val result = schoolService.updateCourse(
          ...
          Some(updatedCourse.archived),
          Some(updatedCourse.schedulingEnabled),
          Some(updatedCourse.theaterMode),
          Some(updatedCourse.lastProjectId),
  ...
  "SchoolService.findUserForTeacher" should {
    inSequence {
      "return RepositoryError.NoResults if user is not in the teacher's class" in {
        ...
        (courseRepository.list(_: User, _: Boolean)(_: Connection)) when (testStudent, false, *) returns (Future.successful(\/-(testCourses)))

        val result = schoolService.findUserForTeacher(testStudent.id, testTeacher.id)
        Await.result(result, Duration.Inf).equals(-\/(RepositoryError.NoResults(s"User ${testStudent.id.toString} is not in any courses wiht teacher ${testTeacher.id.toString}")))
      }
    }
  }
  ...
  "SchoolService.insertChat" should {
    inSequence {
      "insert new chat if user is a teacher" in {
        ...
        (userRepository.find(_: UUID, _: Boolean)(_: Connection)) when (testUser.id, *, *) returns (Future.successful(\/-(testUser)))
        (courseRepository.find(_: UUID)(_: Connection)) when (testCourse.id, *) returns (Future.successful(\/-(testCourse)))
        (userRepository.list(_: Course)(_: Connection)) when (testCourse, *) returns (Future.successful(\/-(studentList)))
        (chatRepository.insert(_: Chat)(_: Connection)) when (*, *) returns (Future.successful(\/-(testChat)))

        val result = schoolService.insertChat(testCourse.id, testUser.id, testChat.message, testChat.shouting)
      ...
      "return ServiceError.BadPermissions if user is not a teacher and doesn't attend the group" in {
        ...
        (courseRepository.find(_: UUID)(_: Connection)) when (testCourse.id, *) returns (Future.successful(\/-(testCourse)))
        (userRepository.list(_: Course)(_: Connection)) when (testCourse, *) returns (Future.successful(\/-(studentList)))
        (chatRepository.insert(_: Chat)(_: Connection)) when (testChat, *) returns (Future.successful(\/-(testChat)))
